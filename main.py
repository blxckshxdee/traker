        ...
        if os.path.exists(DATA_FILE):
            with open(DATA_FILE, "r", encoding="utf-8") as f:
                ...
                if "task_history" not in data:
                    data["task_history"] = {}
                if "shop_limits" not in data:
                    data["shop_limits"] = {}
                if "custom_rewards" not in data:
                    data["custom_rewards"] = []
                if "custom_history" not in data:
                    data["custom_history"] = []
                return data
        return {
...
class CustomRewardsScreen(Screen):
    ...
    def show_wallet_deposit_popup(self, instance):
        ...
        layout.add_widget(
            Label(text="какую сумму ты кладашь?", font_size='13sp',
                  halign='center', color=(0.6, 0.6, 0.6, 1)))
...
class LifeRPGApp(App):
    ...
    def show_windows_toast(self, title, message):
        """Заглушка для toast уведомлений - на Android не работает"""
