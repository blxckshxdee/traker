    ...
    def load_data():
        if os.path.exists(DATA_FILE):
            with open(DATA_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
                for key in ["task_history", "shop_limits", "custom_rewards", "custom_history"]:
                    if key not in data:
                        data[key] = {} if key != "custom_rewards" and key != "custom_history" else []
                return data
        return {
...
class MenuScreen(Screen):
    def __init__(self, **kwargs):
        ...
        layout.add_widget(Label(text="[ туман внутри ]\n+----------------------+", font_size='22sp', bold=True, halign='center', size_hint_y=None, height=80, color=(0.8, 0.8, 0.8, 1)))
        self.stats_label = Label(text="загрузка...", font_size='15sp', halign='center', valign='middle', size_hint_y=None, height=110, color=(0.6, 0.6, 0.6, 1))
        self.stats_label.bind(size=self.stats_label.setter('text_size'))
        layout.add_widget(self.stats_label)

        for btn_text, callback in [("[ задачи ]", self.go_to_tasks), ("[ магазин ]", self.go_to_shop), ("[ копилка ]", self.go_to_custom_rewards)]:
            btn = Button(text=btn_text, font_size='16sp', size_hint_y=None, height=60, background_normal='', background_color=(0.2, 0.22, 0.24, 1), color=(0.8, 0.8, 0.8, 1))
            btn.bind(on_press=callback)
            layout.add_widget(btn)

        self.btn_clean = Button(text=" ", font_size='16sp', size_hint_y=None, height=60, background_normal='', color=(0.8, 0.8, 0.8, 1))
        self.btn_clean.bind(on_press=self.claim_clean_day)
        layout.add_widget(self.btn_clean)

        self.btn_reset = Button(text=" ", font_size='13sp', size_hint_y=None, height=50, background_normal='', color=(0.5, 0.5, 0.5, 1))
    ...
    def get_rank(self, total_earned):
        ranks = [(200, "гуль C ранга"), (600, "гуль B ранга"), (1500, "гуль A ранга"), (2300, "гуль S ранга"), (3200, "гуль S+ ранга"), (5000, "гуль SS ранга")]
        for limit, rank in ranks:
            if total_earned < limit:
                return rank
    ...
    def check_monthly_reset(self):
        ...
        if current_month != app.last_month:
            ...
            DataManager.save_data(app.days_clean, app.coins, app.total_earned, app.last_month, app.task_history, app.shop_limits, app.custom_rewards, app.custom_history)
    ...
    def confirm_reset_popup(self, instance):
        ...
        layout.add_widget(Label(text="ты действительно сорвался?\nэто отбросит твой ранг на -1000 опыта.", font_size='14sp', halign='center', color=(0.8, 0.8, 0.8, 1)))
        ...
        popup = Popup(title="признание слабости", content=layout, size_hint=(0.85, 0.35), title_color=(0.7, 0.7, 0.7, 1), separator_color=(0.3, 0.3, 0.3, 1), background_color=(0.05, 0.05, 0.05, 1))
...
class TasksScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.main_layout.add_widget(Label(text="список испытаний", font_size='20sp', bold=True, size_hint_y=None, height=45, color=(0.8, 0.8, 0.8, 1)))
        ...
        self.categories = {
            "+--- порядок в доме ---+": [("вынести мусор и проветрить", 5, "midnight"), ("помыть посуду", 10, "midnight"), ("уборать комнату)", 20, "midnight")],
            "+--- испытания тела ---+": [("чистая вода натощак", 5, "midnight"), ("умыться и почистить зубы", 10, "midnight"), ("утренняя зарядка и растяжка", 15, "midnight"), ("прогулка", 0, "hours_2"), ("день без сладостей", 40, "midnight"), ("тренировка", 40, "midnight")],
            "+--- испытания разума ---+": [("сделано дз", 30, "midnight"), ("написание кода/изучение python", 35, "midnight"), ("работа", 0, "hours_1"), ("день без ютуба и тт)", 50, "midnight"), ("день воздержания", 50, "midnight")],
            "+--- хобби ---+": [("написать сэмпл", 20, "midnight"), ("написать рисунок/арт", 30, "midnight"), ("написать бит", 40, "midnight")],
            "+--- погружение в сон ---+": [("изоляция от экранов за час до сна", 25, "midnight"), ("встать по первому будильнику", 30, "midnight"), ("встать без будильника", 35, "midnight")]
        }
        ...
        btn_back = Button(text="<-- назад", font_size='14sp', size_hint_y=None, height=55, background_color=(0.15, 0.15, 0.15, 1), background_normal='', color=(0.7, 0.7, 0.7, 1))
    ...
    def create_task_button(self, text, reward, cd_type, is_sleep=False):
        btn = Button(text=" ", font_size='13sp', halign='center', size_hint_y=None, height=65, background_normal='', color=(0.8, 0.8, 0.8, 1))
    ...
    def show_walk_popup(self, task_key):
        ...
        header.add_widget(Label(text="фиксация прогулки", font_size='15sp', bold=True, color=(0.7, 0.7, 0.7, 1), halign='left'))
        btn_close_popup = Button(text="[ X ]", size_hint_x=None, width=50, font_size='14sp', bold=True, background_normal='', background_color=(0, 0, 0, 0), color=(0.8, 0.5, 0.5, 1))
        ...
        inp = TextInput(hint_text="введите минуты...", multiline=False, input_filter="int", size_hint_y=None, height=45, background_normal='', background_color=(0.12, 0.13, 0.15, 1), foreground_color=(1, 1, 1, 1), hint_text_color=(0.5, 0.5, 0.5, 1))
        layout.add_widget(inp)
        btn_confirm = Button(text="[ зачесть ]", size_hint_y=None, height=45, background_normal='', background_color=(0.2, 0.22, 0.24, 1), color=(0.8, 0.8, 0.8, 1))
        layout.add_widget(btn_confirm)
        popup = Popup(title="", content=layout, size_hint=(0.9, 0.42), separator_height=0, background_color=(0.05, 0.05, 0.05, 1))
    ...
    def show_order_popup(self, task_key):
        ...
        state = {"loc": "studio"}
        btn_studio = Button(text="[X] в студии", size_hint_y=None, height=45, background_normal='', background_color=(0.2, 0.22, 0.24, 1), color=(0.8, 0.8, 0.8, 1))
        btn_field = Button(text="выезд (х2)", size_hint_y=None, height=45, background_normal='', background_color=(0.12, 0.13, 0.15, 1), color=(0.5, 0.5, 0.5, 1))
        ...
        inp_hours = TextInput(hint_text="часы...", multiline=False, input_filter="int", background_normal='', background_color=(0.12, 0.13, 0.15, 1), foreground_color=(1, 1, 1, 1), hint_text_color=(0.4, 0.4, 0.4, 1))
        inp_mins = TextInput(hint_text="минуты...", multiline=False, input_filter="int", background_normal='', background_color=(0.12, 0.13, 0.15, 1), foreground_color=(1, 1, 1, 1), hint_text_color=(0.4, 0.4, 0.4, 1))
        time_input_layout.add_widget(inp_hours)
        time_input_layout.add_widget(inp_mins)
        layout.add_widget(time_input_layout)

        btn_confirm = Button(text="[ зафиксировать ]", size_hint_y=None, height=45, background_normal='', background_color=(1, 1, 1, 1), color=(0, 0, 0, 1))
        layout.add_widget(btn_confirm)
        popup = Popup(title="", content=layout, size_hint=(0.9, 0.68), separator_height=0, background_color=(0.05, 0.05, 0.05, 1))
        btn_close_popup.bind(on_press=popup.dismiss)
...
class ShopScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.title_label = Label(text="магазин", font_size='20sp', bold=True, halign='center', size_hint_y=None, height=60, color=(0.8, 0.8, 0.8, 1))
        ...
        self.time_input = TextInput(text='', hint_text='введите количество минут игры...', multiline=False, input_filter='int', size_hint_y=None, height=45, background_color=(0.12, 0.13, 0.15, 1), foreground_color=(1, 1, 1, 1), hint_text_color=(0.4, 0.4, 0.4, 1))
        self.layout_inside.add_widget(self.time_input)
        btn_buy_time = Button(text="[ выкупить время игры (1 мин = 2) ]", font_size='14sp', size_hint_y=None, height=55, background_normal='', background_color=(0.2, 0.22, 0.24, 1), color=(0.8, 0.8, 0.8, 1))
        ...
        for name, cost, item_id, cd_info in self.rewards_data:
            btn = Button(text=" ", font_size='13sp', halign='center', size_hint_y=None, height=55, background_normal='', color=(0.8, 0.8, 0.8, 1))
    ...
    def update_shop_ui(self, dt):
        ...
        self.time_status_label.text = f"--- купить игровое время ---\nосталось играть: {play_time_str}\nсегодня выкуплено: {spent_today} / 180 мин.\n[ сброс лимита через: {time_until_midnight} ]"
    ...
    def show_time_purchase_popup(self, item_id):
        ...
        layout = BoxLayout(orientation='vertical', padding=15, spacing=10)
        header = BoxLayout(orientation='horizontal', size_hint_y=None, height=40)
        header.add_widget(Label(text=f"контракт: {type_label}", font_size='15sp', bold=True, color=(0.7, 0.7, 0.7, 1), halign='left'))
        btn_close_popup = Button(text="[ X ]", size_hint_x=None, width=50, font_size='14sp', bold=True, background_normal='', background_color=(0, 0, 0, 0), color=(0.8, 0.5, 0.5, 1))
        header.add_widget(btn_close_popup)
        layout.add_widget(header)
        layout.add_widget(Label(text=f"сколько минут {type_label} покупаешь?\n[ курс: 1 минута = 1 осколок]", font_size='13sp', halign='center', color=(0.6, 0.6, 0.6, 1)))
        inp = TextInput(hint_text="число минут...", multiline=False, input_filter="int", size_hint_y=None, height=45, background_normal='', background_color=(0.12, 0.13, 0.15, 1), foreground_color=(1, 1, 1, 1), hint_text_color=(0.5, 0.5, 0.5, 1))
        layout.add_widget(inp)
        btn_confirm = Button(text="[ подтвердить покупку ]", size_hint_y=None, height=45, background_normal='', background_color=(0.2, 0.22, 0.24, 1), color=(0.8, 0.8, 0.8, 1))
        layout.add_widget(btn_confirm)
        popup = Popup(title="", content=layout, size_hint=(0.9, 0.42), separator_height=0, background_color=(0.05, 0.05, 0.05, 1))
        btn_close_popup.bind(on_press=popup.dismiss)
    ...
    def show_no_coins_popup(self, missing_amount):
        ...
        layout = BoxLayout(orientation='vertical', padding=15, spacing=10)
        header = BoxLayout(orientation='horizontal', size_hint_y=None, height=40)
        header.add_widget(Label(text="отказ магазина", font_size='15sp', bold=True, color=(0.7, 0.7, 0.7, 1), halign='left'))
        btn_close_popup = Button(text="[ X ]", size_hint_x=None, width=50, font_size='14sp', bold=True, background_normal='', background_color=(0, 0, 0, 0), color=(0.8, 0.5, 0.5, 1))
        header.add_widget(btn_close_popup)
        layout.add_widget(header)
        layout.add_widget(Label(text=f"недостаточно осколков.\nнужно еще: {missing_amount}", font_size='14sp', halign='center', color=(0.6, 0.6, 0.6, 1)))
        btn_ok = Button(text="[ ок ]", size_hint_y=None, height=45, background_normal='', background_color=(1, 1, 1, 1), color=(0, 0, 0, 1))
        layout.add_widget(btn_ok)
        popup = Popup(title="", content=layout, size_hint=(0.85, 0.36), separator_height=0, background_color=(0.05, 0.05, 0.05, 1))
        btn_close_popup.bind(on_press=popup.dismiss)
...
class CustomRewardsScreen(Screen):
    def __init__(self, **kwargs):
        ...
        btn_add_to_wallet = Button(text="[ + пополнить конверт ]", font_size='14sp', bold=True, background_normal='', background_color=(0.15, 0.25, 0.15, 1), color=(0.8, 1, 0.8, 1))
        ...
        self.btn_tab_active = Button(text="[ конверт: наличные ]", background_normal='', background_color=(0.15, 0.16, 0.18, 1), color=(1, 1, 1, 1))
        self.btn_tab_history = Button(text="архив покупок", background_normal='', background_color=(0.08, 0.08, 0.08, 1), color=(0.5, 0.5, 0.5, 1))
        ...
        self.name_in = TextInput(hint_text="название...", multiline=False, size_hint_y=None, height=38, background_normal='', background_color=(0.12, 0.13, 0.15, 1), foreground_color=(1, 1, 1, 1), hint_text_color=(0.4, 0.4, 0.4, 1))
        self.price_in = TextInput(hint_text="цена...", multiline=False, input_filter="int", size_hint_y=None, height=38, background_normal='', background_color=(0.12, 0.13, 0.15, 1), foreground_color=(1, 1, 1, 1), hint_text_color=(0.4, 0.4, 0.4, 1))
        self.btn_photo_pick = Button(text="[ выбрать фото ]", size_hint_y=None, height=38, background_normal='', background_color=(0.14, 0.16, 0.18, 1), color=(0.6, 0.7, 0.6, 1))
        self.btn_photo_pick.bind(on_press=self.open_file_chooser)
        btn_add = Button(text="[ + ] новая цель", font_size='14sp', size_hint_y=None, height=45, background_normal='', background_color=(0.2, 0.22, 0.24, 1), color=(0.8, 0.8, 0.8, 1))
        ...
        btn_back = Button(text="<-- назад", size_hint_y=None, height=55, background_normal='', background_color=(0.15, 0.15, 0.15, 1), color=(0.7, 0.7, 0.7, 1))
        btn_back.bind(on_press=self.go_back)
        self.main_layout.add_widget(btn_back)
        self.add_widget(self.main_layout)
        ...
        for index, item in enumerate(current_list):
            ...
            if self.current_tab == "active":
                ...
                if wallet_balance >= price:
                    btn_buy = Button(text="[ купил ]", size_hint_x=None, width=100, font_size='13sp', bold=True, background_normal='', background_color=(0.15, 0.35, 0.15, 1), color=(0.8, 1, 0.8, 1))
                    btn_buy.bind(on_press=lambda inst, idx=index: self.archive_item(idx))
                else:
                    btn_buy = Button(text="коплю...", size_hint_x=None, width=100, font_size='13sp', disabled=True, background_normal='', background_color=(0.08, 0.08, 0.08, 1), color=(0.4, 0.4, 0.4, 1))
                card.add_widget(btn_buy)
            else:
                info_box.add_widget(Label(text=f"цена: {price} руб.", font_size='13sp', color=(0.5, 0.5, 0.5, 1), halign='left'))
                card.add_widget(info_box)
                done_date = item.get("date", "уплено")
    ...
    def show_wallet_deposit_popup(self, instance):
        ...
        btn_close = Button(text="[ X ]", size_hint_x=None, width=50, font_size='14sp', bold=True, background_normal='', background_color=(0, 0, 0, 0), color=(0.8, 0.5, 0.5, 1))
        header.add_widget(btn_close)
        layout.add_widget(header)
        layout.add_widget(Label(text="какую сумму ты кладаешь?", font_size='13sp', halign='center', color=(0.6, 0.6, 0.6, 1)))
        inp = TextInput(hint_text="сумма пополнения...", multiline=False, input_filter="int", size_hint_y=None, height=45, background_normal='', background_color=(0.12, 0.13, 0.15, 1), foreground_color=(1, 1, 1, 1), hint_text_color=(0.5, 0.5, 0.5, 1))
        layout.add_widget(inp)
        btn_confirm = Button(text="[ подтвердить ]", size_hint_y=None, height=45, background_normal='', background_color=(1, 1, 1, 1), color=(0, 0, 0, 1))
        layout.add_widget(btn_confirm)
        popup = Popup(title="", content=layout, size_hint=(0.9, 0.42), separator_height=0, background_color=(0.05, 0.05, 0.05, 1))
    ...
    def open_file_chooser(self, instance):
        ...
        btn_close = Button(text="[ X ]", size_hint_x=None, width=50, background_normal='', background_color=(0, 0, 0, 0), color=(0.8, 0.5, 0.5, 1))
        header.add_widget(btn_close)
        ...
        btn_select = Button(text="[ открыть ]", size_hint_y=None, height=50, font_size='15sp', background_normal='', background_color=(1, 1, 1, 1), color=(0, 0, 0, 1))
        popup_layout.add_widget(btn_select)
        popup = Popup(title="", content=popup_layout, size_hint=(0.95, 0.95), separator_height=0, background_color=(0.05, 0.05, 0.05, 1))
        btn_close.bind(on_press=popup.dismiss)
    ...
    def open_hardware_editor_popup(self, img_path):
        ...
        btn_close = Button(text="[ X ]", size_hint_x=None, width=50, font_size='15sp', bold=True, background_normal='', background_color=(0, 0, 0, 0), color=(0.8, 0.5, 0.5, 1))
        header.add_widget(btn_close)
        ...
        editor_layout.add_widget(Label(text="область обрезки:", font_size='11sp', color=(0.5, 0.5, 0.5, 1), size_hint_y=None, height=12))
        ...
        editor_layout.add_widget(Label(text="сдвиг по горизонтали:", font_size='11sp', color=(0.5, 0.5, 0.5, 1), size_hint_y=None, height=12))
        ...
        editor_layout.add_widget(Label(text="сдвиг по вертикали:", font_size='11sp', color=(0.5, 0.5, 0.5, 1), size_hint_y=None, height=12))
        ...
        btn_crop = Button(text="[ сохранить ]", size_hint_y=None, height=45, font_size='14sp', background_normal='', background_color=(1, 1, 1, 1), color=(0, 0, 0, 1))
        editor_layout.add_widget(btn_crop)
        popup = Popup(title="", content=editor_layout, size_hint=(0.95, 0.95), separator_height=0, background_color=(0.05, 0.05, 0.05, 1))
        btn_close.bind(on_press=popup.dismiss)
...
class LifeRPGApp(App):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.morning_alerts = ["время действовать", "начни день правильно", "твой путь начинается сейчас"]
        self.day_alerts = ["не сбавляй темп", "половина дня позади", "продолжай в том же духе"]
        self.evening_alerts = ["заверши день достойно", "время подвести итоги", "не放松ись сейчас"]
